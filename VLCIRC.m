...
function [ARRAY_LENGTH] = get_array_len(Room,del_t,MAX_BOUNCE)
    ...
    % NOTE: MAX_BOUNCE is added to avoid out of range errors since each
    % bounce moves the signal at least 1 time step. For low time resolution
    % the minimum of 1 time step per bounce will cause errors in the 
    % impulse response; but low time resolution is intended for getting 
    % accurate power results faster.
    ARRAY_LENGTH = MAX_BOUNCE + ...
                   ceil((MAX_BOUNCE+1)*sqrt(Room.length^2 + ...
                                            Room.width^2  +  ...
                                            Room.height^2) ...
